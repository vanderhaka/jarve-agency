...
export function V2PortfolioSection() {
  const clientWork = [
    {
      title: 'Diggable',
  ...
  const renderCard = (project: typeof clientWork[0]) => (
    <Card className="overflow-hidden group border-none shadow-none bg-transparent h-full flex flex-col rounded-none">
      <div className="aspect-video relative rounded-xl overflow-hidden bg-muted mb-6 border border-[hsl(35,15%,82%)] shadow-sm group-hover:shadow-md transition-all">
      ...
      <CardContent className="p-0 flex-1 flex flex-col">
        <div className="flex items-center justify-between mb-2">
          <div className="text-sm font-medium text-[hsl(140,18%,38%)]">{project.category}</div>
          {project.href && (
            <ExternalLink className="w-4 h-4 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
          )}
        </div>
        <h3 className="text-2xl font-bold mb-3 group-hover:text-[hsl(140,18%,33%)] transition-colors">
          {project.title}
        </h3>
        ...
        <div className="mt-auto">
          <span className="flex items-center justify-center w-full px-4 py-2 bg-[hsl(140,18%,38%)] text-sm font-medium text-white rounded-md">
            {project.result}
          </span>
        </div>
      </CardContent>
  ...
  return (
    <section className="py-24 px-4 bg-gradient-to-b from-[hsl(35,15%,93%)] to-background" id="portfolio">
          ...
          {clientWork.map((project, index) => (
            <FadeIn key={project.title} delay={0.2 + index * 0.1} className="h-full">
              {project.href ? (
                ...
                <div className="h-full">{renderCard(project)}</div>
              )}
            </FadeIn>
          ))}
        </div>
