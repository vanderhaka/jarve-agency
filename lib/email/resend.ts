...
export interface SendPortalLinkEmailParams {
  to: string
  recipientName: string
  projectName: string
  portalUrl: string
...
export async function sendPortalLinkEmail({
  ...
  const { data, error } = await resend.emails.send({
    from: 'JARVE <portal@jarve.com.au>',
    to,
    subject: `Your Client Portal - ${projectName}`,
    html: `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #1a1a1a; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <p style="margin: 0 0 16px 0;">
            Here's the link to your Client Portal for <strong>${projectName}</strong>.
          </p>

          <p style="margin: 0 0 24px 0;">
            Your portal is your home base for everything project-related — signed documents, messaging, and file sharing.
          </p>
...
Here's the link to your Client Portal for ${projectName}.
...
`
  ...
  if (error) {
    console.error('[sendPortalLinkEmail] Error:', error)
    throw error
  }
